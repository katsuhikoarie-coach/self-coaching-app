...
└── automation/       ← ヤマノ発注自動化スクリプト
    └── yamano-scraper.js  ← ヤマノ注文履歴取込スクリプト（新規）
```
...
**yamano_shipping_map**（配送先→センター紐づけマスタ　※新規）
| カラム | 内容 |
|--------|------|
| id | 自動採番 |
...
現状は月末にヤマノからExcelデータをもらうまで数字が把握できない。
これを解消するため、クロコが毎日自動でヤマノの注文履歴を取込んでDBに保存する。

### 取込スクリプト

- ファイル：`automation/yamano-scraper.js`
- 使用技術：Playwright（既存の自動化と同じ）
- 実行タイミング：毎日1回（Windowsタスクスケジューラで自動実行）
...
　その場合は未分類として管理画面に表示され、在りさんが手動でセンターに紐づける。
　紐づけ後はyamano_shipping_mapに自動登録して次回から自動マッチング。

### 管理画面への追加タブ「ヤマノ実績」

管理画面（/admin/）に「ヤマノ実績」タブを新設する。

表示内容：
- 月度フィルター（デフォルト：当月度）
- センター別集計テーブル（センター名・件数・合計金額）
- 朝霧ヤマノ全体の合計
- 未分類一覧（注文番号・日付・配送先氏名・金額・紐づけボタン）

未分類の手動紐づけUI：
- 未分類行の「紐づける」ボタンを押すとセンター選択ドロップダウンが出る
- センターを選択して確定すると `yamano_orders.fc_user_id` を更新
- 同時に `yamano_shipping_map` にその氏名を登録（次回から自動マッチング）

### 関連APIファイル（新規作成）

| ファイル | 内容 |
|---------|------|
| `api/admin/yamano-orders.php` | ヤマノ実績一覧・センター別集計取得 |
| `api/admin/yamano-map.php` | 未分類の手動紐づけ・マッピング登録 |

### SQLファイル（新規作成）

| ファイル | 内容 |
|---------|------|
| `sql/create_yamano_tables.sql` | yamano_orders・yamano_order_items・yamano_shipping_mapのテーブル定義 |
...
- 件数：380品目 → 450品目（2026年5月22日に一般カテゴリ70品目追加）
- ヤマノ正式商品コードを使用
- CS・販促品はFC価格・BC価格・販社価格を3列で登録
- 一般商品はFC・BC・販社すべて同一価格
...
### カテゴリ一覧
スキンケア / メイク / ボディ&ヘアケア / エステキープ / 健康食品 / キット / CS商品 / 販促品 / **一般**

### SQLファイル
- `sql/create_products_table.sql` ← テーブル定義
- `sql/insert_products.sql` ← 初期データ投入（ON DUPLICATE KEY UPDATE）
- `sql/insert_ippan_products.sql` ← 一般カテゴリ70品目（2026年5月22日追加）
...
- 商品管理（商品一覧・追加・編集・無効化）← 2026年5月20日追加
- CSVダウンロード（月度指定、Shift-JIS）
- ヤマノ実績（センター別集計・未分類紐づけ）← 2026年5月21日追加予定
...
### 2026年5月22日
- 商品カテゴリ「一般」追加（美顔器・電解水素水・琥珀の夢布団・エアフット・パフ・各種パーツ）
- `src/lib/products.ts` のCategory型・CATEGORIES配列に「一般」追加
- `sql/insert_ippan_products.sql` 新規作成（70品目・FC/BC/販社同一価格）
- ビルド完了・WinSCPアップ済み
- phpMyAdminでのSQL実行：在りさんが実施

### 2026年5月21日
- ヤマノ注文履歴取込の設計（yamano_orders・yamano_order_items・yamano_shipping_map）
- 管理画面「ヤマノ実績」タブの設計
...
### ヤマノ実績取込（優先度高）
- yamano_orders・yamano_order_items・yamano_shipping_map テーブル作成
- automation/yamano-scraper.js 作成（Playwrightで毎日自動取込）
- Windowsタスクスケジューラに登録
- 管理画面に「ヤマノ実績」タブ追加（センター別集計・未分類紐づけUI）
- api/admin/yamano-orders.php・yamano-map.php 作成
